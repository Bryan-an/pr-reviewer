...
Engines implement `ReviewEngine` (defined in `server/ai/engine.ts`). Input: PR metadata + local repo dir + unified diff. Output: structured findings (including optional `lineStart`/`lineEnd` for line-level positioning). Implementations: `coderabbit/coderabbit-engine.ts` (general quality review), `claude-code/claude-code-engine.ts` (rule compliance via Claude Code CLI), `stub-engine.ts` (testing). Selected via `REVIEW_ENGINE` env var (`coderabbit` default, `claude-code`, `stub`). Engine name enum (`REVIEW_ENGINE` const, `ReviewEngineName` type, `reviewEngineNameValues` array) lives in `lib/validation/review-engine-name.ts` (shared, not server-only) — `lib/config/env.ts` imports from it for Zod validation. `selectEngineConfig()` in `server/ai/select-engine.ts` returns `{ engines, configuredName: ReviewEngineName }`.

### Parallel engine execution

By default (`REVIEW_ENGINE=coderabbit`), both CodeRabbit and Claude Code engines run in parallel via `Promise.allSettled` (`server/ai/run-engines-in-parallel.ts`). Claude Code self-gates: if the repo has no enabled rules, it returns empty findings (not an error). Each finding is stamped with `sourceName` (typed `ReviewEngineName`) by the orchestrator. The run-level `engineName` comes from `selectEngineConfig().configuredName` (the env-configured engine), not from concatenating succeeded engine names. After parallel execution, AI-powered deduplication (`server/ai/dedup/deduplicate-findings.ts`) merges near-duplicate findings across engines. If one engine fails, partial results from the other are used. `AllEnginesFailedError` is thrown only when ALL engines fail.
...
- `ReviewRun` → `Finding[]` (review execution + results). Each `Finding` has a `status` column (`pending`/`published`/`ignored`) — see `FindingStatus` in `lib/validation/finding-status.ts`. `sourceName` (optional, typed `ReviewEngineName`) tracks which engine produced the finding (displayed in UI, not published to ADO threads). Prisma returns `string` from SQLite — `toDomainResult` in `server/db/review-runs.ts` casts `engineName` and `sourceName` to `ReviewEngineName` at the domain boundary. `codeSnippet` (optional) stores raw diff lines extracted from the parsed diff at review time (`server/review/extract-code-snippet.ts`). Pipeline enrichment — not in `FindingSchema` (which validates both AI engine output and cached DB findings). Rendered as fenced `diff` code blocks via `rehype-highlight`. `adoThreadId` (optional `Int`) stores the ADO thread ID after publishing — used by restore to close threads. Domain `Finding` type uses `findingKey` (content-derived, always present — generated by `stableFindingKey()` in each engine) and `id` (database PK, optional — absent for fresh AI output, populated after persistence). The UI `FindingWithStatus` mirrors this: `id` (always present, from DB) and `findingKey`. `findingKey` maps to Prisma `findingKey` column; `id` maps to Prisma `id` (cuid PK)
- `Repository` → `RepoRule[]` (per-repo markdown review rules, managed via `/repos` UI)

### Azure DevOps thread anchoring

Publishing lives in `server/review/publish/`: `format-threads.ts` (pure formatting → `PublishableThread[]`), `publish-review.ts` (orchestration: fetch PR, format, deduplicate via HTML-comment markers, publish loop with fallback, reopen closed threads on re-publish), `close-threads.ts` (domain layer for closing/bulk-closing threads on restore), `threads.ts` in `server/azure-devops/` (ADO API calls — `createPullRequestThread`, `closePullRequestThread`, `reopenPullRequestThread`, `listPullRequestThreads`). `formatFinding()` renders message as a blockquote and recommendation as a fenced code block (via `adoFencedBlock()` in `ado-markdown.ts`) — ADO renders fenced blocks with a built-in copy button. `adoFencedBlock` uses dynamic fence length (like `adoInlineCode`) to safely handle recommendations containing backtick runs. Thread types: general findings (unscoped, no file path) and per-finding (file-scoped, line-anchored). Idempotency uses `<!-- pr-reviewer:thread:... -->` markers embedded in comment content. `PublishableThread.findingKeys` tracks which finding keys are in each thread. Individual finding actions (publish/ignore/restore) are server actions in `app/review/_actions/finding-actions.ts`; bulk publish is in `app/review/_actions/publish-action.ts` (skips already-published and ignored findings); both single and bulk publish check `result.processedFindings` before marking findings as published in the DB. bulk restore-all is in `app/review/_actions/restore-all-action.ts` (resets published/ignored findings to pending and closes ADO threads for published findings via best-effort bulk marker scan). Re-run is in `app/review/_actions/rerun-action.ts`. All review-feature server actions use typed argument objects (not `FormData`) — `FormData` is only used by the home page form (`app/page.tsx`) which needs native `<form action={...}>` submission. Finding status is persisted via `server/db/findings.ts`.
...
- **Header actions**: `PageHeader` action links use `variant: "outline"` + `size: "sm"`; headers contain only navigation (e.g., "Back"), not duplicate page actions. `PageHeader` `title` accepts `ReactNode` — pass `<Skeleton>` for loading states where the page title is async
...
- **Integration layer error logging**: functions in `server/azure-devops/`, `server/git/`, and `server/ai/` that call external APIs must wrap calls in try/catch, log with `logger.error(err, "descriptive message")`, and re-throw — the integration handles observability, callers handle error UX (error boundary, toast, fallback)
- **Server action error logging**: all server actions must log errors with `logger.error(err, "[actionName] failed")`. Wrap the entire action body in a top-level try/catch — not just mutations — so query errors (e.g., DB lookups) are also caught and logged. Actions returning discriminated unions: catch + log + return `{ success: false }`. Actions that throw (native `<form action={...}>` consumption): catch + log + re-throw to preserve the error boundary contract. Best-effort (non-fatal) operations within an action get their own nested try/catch with `logger.warn(err, "[actionName] descriptive message (non-fatal)")` and continue execution. Guard clauses for missing/invalid input use `logger.warn` (not `error`) — include the offending value as context, e.g., `logger.warn({ runId }, "[actionName] invalid runId format")`
...
- **Server Component error handling**: pages calling external APIs (Azure DevOps) should wrap calls in `try/catch`, log with `logger.error(err, "[PageName] descriptive message")` (bracket prefix matching server action convention), and re-throw — let the nearest `error.tsx` boundary render the error UI. The page handles observability (structured log with route context); `error.tsx` handles user-facing recovery (Back link + Try again). `error.tsx` must be a Client Component (Next.js requirement) and must NOT log errors (pages already log server-side with full context). A parent `error.tsx` catches errors from all descendant pages — no need for per-page boundaries unless the error UI must differ. Existing boundaries: `app/repos/error.tsx` (repos list page), `app/repos/[org]/[project]/[adoRepoId]/error.tsx` (all pages in that segment)
...
- **Prose `<pre>` sizing**: `text-xs` wrapper yields line-height 18px and 9px padding per side (em cascade through `@tailwindcss/typography`). When computing heights for a clipped `<pre>` (`overflow-hidden` + `max-height`), only `padding-top` is visible — `padding-bottom` renders at the end of full content, outside the clip. `CodeSnippetBlock` in `app/review/_components/finding-card.tsx` encodes these as named constants (`SNIPPET_LINE_HEIGHT_PX`, `SNIPPET_PADDING_TOP_PX`, etc.) with the derivation documented inline.
