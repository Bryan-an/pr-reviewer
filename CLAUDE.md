...
- **Markdown rendering**: `components/markdown.tsx` uses `prose prose-sm max-w-none` + `rehype-highlight` (sync). Prose token colors mapped to design system in `globals.css`. hljs syntax theme (GitHub Light / Dark Dimmed) must be unlayered CSS to override prose's layered `pre code` color rules. Use `rehype-highlight` (not Shiki) — Shiki is async and incompatible with `"use client"` components
